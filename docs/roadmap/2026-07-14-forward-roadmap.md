...
B2. **Instant NPC minting.** ✅ done 2026-07-14 (commit `f706a10`). `npcModel.mintNpc(rng)` draws one from each tiny local table (name / role / motive / secret + random attitude) → a full `SocialNpc`. A "✨ Mint NPC" button in the Social "People in the scene" panel drops the generated NPC into the scene (persisted with the screen, survives reload — no capture needed); each NPC card also gained a ✕ remove (pre-empting the "add but can't remove" gap the DM flagged in combat). 4 unit/component tests + `test:ci` **127 green**; browser+axe verified (mint → card with name/role/wants/secret, remove, 0 violations). Budget fine after B0 (index 177/560, CSS 109/130).
   ...
   - ⏳ **deferred** (logged): in-place **re-roll** of a minted NPC's identity (DM P1 — attitude already cycles cheaply; identity should too, vs remove-and-mint); a persistent **"improvised"** marker distinguishing filler from prepped NPCs beyond the transient highlight (designer P1); dedupe/cap on repeated mints (P2); and the full-vision **linkable Entity** + "voice" field (foundation work).
