...
C3. **Read-aloud + presenter.** ✅ done 2026-07-14 (commit `1642386`). Extended the existing static `.read-aloud` box in the Exploration ledger (no parallel model — spine lesson): made the boxed prose **editable** (`exploration.readAloud`, persisted; hydrate-merged for old screens) and added a one-tap **"Present"** → a full-screen presenter overlay (navy bg, large centred parchment Cinzel `clamp(28–50px)`, Close). 1 component test + `test:ci` **131 green**; browser+axe 0 violations. **Incidentally fixed** a pre-existing AA bug it surfaced: completed exploration moments were dimmed with `opacity:.67`, dropping their 9px status label to 2.64:1 → now a strike-through title keeps text at full contrast.
   ...
   - ⏳ **deferred** (documented v1 cut, both judges P1): read-aloud is **Exploration-only** — boxed text is also wanted at Narrative/Social scene openers; belongs on `workspace.universal` long-term. A **passage picker / list** beyond per-moment (P2).
