...
4e. **Add combatants to the Run encounter (DM P0, 2026-07-14) — the real on-ramp; do before step 5.** The batch/inline init tools polish a list you currently can't build: in Run mode the **only** way to add a combatant is the PC roster picker; the monsters in the demo are hardcoded fixture data (`focusModel.ts:101-109`), and the legacy Prepare `InitiativeTracker` add-form writes to a disconnected store (`useWidgetStore`) that never reaches `contexts.combat`. Need an **add-NPC/monster form in Run** writing to `encounterModel`/`addCombatant` (name + optional AC/HP/init; `EncounterCombatant.id` is already a free string so duplicate names are structurally fine). Then evaluate the DM's grouped-monsters follow-up (per-instance rows / a quantity concept for "4 goblins").
