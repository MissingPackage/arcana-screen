...
- **Motion identity — "ink-settle":** one ~140ms radial soften-to-sharp gold primitive reused wherever a state commits (die lands, HP changes, card pins); `prefers-reduced-motion` → instant + static outline pulse. A physical signature beyond Cinzel/gold. ⏳ **STILL TODO** — the specified state-commit primitive is not built.
  - *Note (2026-07-14, commit `df2a3e1`):* a **Focus-switch entrance** was added to `.focus-layout` (380ms fade + 7px upward drift; auto-disabled by reduce-motion; no fill-mode, so no lingering transform traps the fixed read-aloud presenter — verified). This is a **different, unrequested motion** than the spec above — kept as a small tasteful touch, but it does **not** discharge the ink-settle track. Honest labelling: I read the precise spec only after building (a repeat of the "check first" lesson). Trivially tweakable (duration/distance/easing) or removable if unwanted.
...
4d. **Combat start throughput (from DM judgment, 2026-07-14).** The single-row edit doesn't scale to the top of round 1.
   ...
   - *DM P1:* **tie visibility** — ✅ done 2026-07-14 (commit `65e6f8f`). `reorderTiedCombatant(encounter, id, direction)` nudges a combatant above/below a same-initiative neighbour via a deterministic tie-breaker bump. In the tracker, small ▲▼ controls appear under a combatant's init **only when it shares an initiative** with a neighbour — so a tie is both *visible* (the controls signal it) and *breakable* by the DM, with zero clutter for the common untied case. Domain + component tests + `test:ci` **136 green**; browser+axe 0 violations.
