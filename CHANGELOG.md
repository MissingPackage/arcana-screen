...
- Dark theme, Run cockpit: the remaining hardcoded ink tints, gold washes and action-link colors (saved-note, read-aloud, session-flow actions, scene-beat numbering, active/current surfaces) now flow from the token layer; an axe scan of Narrative/Exploration/Combat in dark theme reports zero cockpit contrast violations (only the pre-existing header mode-toggle remains, tracked separately).
